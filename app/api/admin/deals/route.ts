...
import {normalizeClosingIso,nextBusinessDate,pacificDateTimeParts,pacificIso} from "../../../../lib/businessDays";
...
export async function GET(request:Request){
  ...
  const uploadIds=deals.map(deal=>deal.source_upload_id).filter(Boolean) as string[];const owners=new Map<string,{name:string;email:string;vendor:string;vendorId:string;lineCount:number;tabCount:number}>();
  if(uploadIds.length){const uploadResponse=await upstream(`/rest/v1/pdd_deal_uploads?select=id,employee_email,vendor_id,column_mapping,quantified_line_count,vendor:pdd_vendors(company_name)&id=in.(${uploadIds.map(encodeURIComponent).join(",")})`,token);if(uploadResponse.ok){const uploads=await uploadResponse.json() as {id:string;employee_email:string;vendor_id?:string|null;column_mapping?:{sheets?:{names?:string[]}}|null;quantified_line_count?:number|null;vendor:{company_name:string}|null}[];let names=new Map<string,string>();const profileResponse=await upstream("/rest/v1/pdd_employee_access?select=email,display_name",token);if(profileResponse.ok)names=new Map((await profileResponse.json() as {email:string;display_name:string}[]).map(profile=>[profile.email,profile.display_name]));for(const upload of uploads){const sheetNames=upload.column_mapping?.sheets?.names;owners.set(upload.id,{name:names.get(upload.employee_email)||upload.employee_email,email:upload.employee_email,vendor:upload.vendor?.company_name||"Unassigned",vendorId:upload.vendor_id||"",lineCount:Number(upload.quantified_line_count||0),tabCount:Array.isArray(sheetNames)&&sheetNames.length?sheetNames.length:1})}}}
  const ownedDeals=deals.map(deal=>{const owner=deal.source_upload_id?owners.get(deal.source_upload_id):undefined;const publicLineCount=Array.isArray(deal.public_lines)?deal.public_lines.length:0;return {...deal,owner_name:owner?.name||"Unassigned",owner_email:owner?.email||"",vendor_name:owner?.vendor||"Unassigned",vendor_id:owner?.vendorId||"",line_count:owner?.lineCount||publicLineCount||1,tab_count:owner?.tabCount||1}});
...
export async function PATCH(request:Request){
  ...
  if(body.commentId){
    ...
    await env.DB.prepare(commentSchema).run();const existing=await env.DB.prepare("SELECT id FROM deal_comments WHERE id=? LIMIT 1").bind(body.commentId).first<{id:string}>();
  ...
  if(body.id&&body.closesAt!==undefined){
    const close=new Date(body.closesAt);if(!Number.isFinite(close.getTime()))return Response.json({error:"Enter a valid closing date and time."},{status:400});const normalizedClose=new Date(normalizeClosingIso(close.toISOString()));
    ...
    const closesAt=normalizedClose.toISOString(),updatedAt=new Date().toISOString(),reopens=current.status==="working"&&normalizedClose.getTime()>Date.now(),response=await upstream(`/rest/v1/pdd_public_deals?id=eq.${encodeURIComponent(body.id)}`,token,{method:"PATCH",headers:{Prefer:"return=representation"},body:JSON.stringify({closes_at:closesAt,...(reopens?{status:"open"}:{}),published:true,updated_at:updatedAt})});if(!response.ok)return Response.json({error:"The closing date and time could not be updated."},{status:502});
    if(current.source_upload_id){const displayDate=new Intl.DateTimeFormat("en-US",{timeZone:"America/Los_Angeles",month:"short",day:"numeric",year:"numeric"}).format(normalizedClose),displayTime=new Intl.DateTimeFormat("en-US",{timeZone:"America/Los_Angeles",hour:"numeric",minute:"2-digit"}).format(normalizedClose),inputParts=pacificDateTimeParts(normalizedClose),dealName=current.category||current.title.replace(/^\d[\d,]*-Piece\s+/i,"").replace(/\s+Lot$/i,""),displayName=`${current.deal_number} — ${Number(current.quantity).toLocaleString("en-US")} pcs ${dealName} — Closes ${displayDate} at ${displayTime} PT`;await upstream(`/rest/v1/pdd_deal_uploads?id=eq.${encodeURIComponent(current.source_upload_id)}`,token,{method:"PATCH",body:JSON.stringify({bid_close_date:inputParts.date,bid_close_time:inputParts.time,display_name:displayName,updated_at:updatedAt})})}
  ...
  if(body.status==="open"){const suggested=new Date(Date.now()+48*60*60*1000),parts=pacificDateTimeParts(suggested);statusUpdate.closes_at=pacificIso(nextBusinessDate(parts.date),parts.time)}
