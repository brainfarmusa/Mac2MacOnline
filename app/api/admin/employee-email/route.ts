...
export async function POST(request:Request){
  ...
  const html=`<h2>${subject}</h2><p>Hello ${escapeHtml(displayName)},</p><p>${recovery?"Use the secure button below to choose a new password for your employee account.":"Your Mac2MacOnline employee access has been approved. Use the secure button below to create your password."}</p><p><a href="${escapeHtml(action.toString())}" style="display:inline-block;padding:12px 18px;background:#176aa6;color:#fff;text-decoration:none;border-radius:6px;font-weight:700">${button}</a></p><p>This link is for the Mac2MacOnline employee Dashboard. If you did not request it, you can ignore this email.</p>`;
