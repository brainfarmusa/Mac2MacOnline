...
export async function GET(request: Request) {
  ...
  return Response.json(
    {
      deals: deals.results || [],
      nextPoNumber: nextPoNumber(deals.results || []),
      items: (items.results || []).map((row: any) => ({
        ...row,
        tech_data: JSON.parse(String(row.tech_data_json || "{}")),
        bitraser_data: JSON.parse(String(row.bitraser_data_json || "{}")),
      })),
    },
    { headers },
  );
  ...
  try {
    ...
    if (action === "create_deal") {
      ...
      const id = crypto.randomUUID(),
        uploadName = clean(body.uploadName, 220),
        notes = [
          clean(body.notes, 1800),
          uploadName ? `Original inventory: ${uploadName}` : "",
        ]
          .filter(Boolean)
          .join("\n"),
        rawItems = Array.isArray(body.items) ? body.items.slice(0, 5000) : [],
        seen = new Set<string>(),
        items = rawItems
          .map((row: any) => ({
            serialNumber: clean(row?.serialNumber, 160).toUpperCase(),
            modelSku: clean(row?.modelSku, 500),
          }))
          .filter(
            (row) =>
              row.serialNumber &&
              !seen.has(row.serialNumber) &&
      ...
      for (let offset = 0; offset < items.length; offset += 75)
        await env.DB.batch(
          items
            ...
            .map((row) =>
              env.DB.prepare(
                "INSERT OR IGNORE INTO r2_processing_items (id,deal_id,serial_number,technician,model_sku,tech_data_json,bitraser_report_id,bitraser_data_json,status,created_at,updated_at) VALUES (?,?,?,?,?,'{}','','{}','testing',?,?)",
              ).bind(
                crypto.randomUUID(),
                id,
                row.serialNumber,
      ...
      return Response.json(
        { ok: true, id, poNumber, itemCount: items.length },
        { headers },
      );
    }
    if (action === "save_item") {
      const dealId = clean(body.dealId, 80),
        serialNumber = clean(body.serialNumber, 160).toUpperCase(),
        technician = clean(body.technician, 160) || employee.displayName,
        status = clean(body.status, 30),
        techData =
          body.techData && typeof body.techData === "object"
            ? body.techData
            : {};
      if (!dealId || !serialNumber || !itemStatuses.has(status))
        return Response.json(
          { error: "Deal, serial number and item status are required." },
          { status: 400, headers },
        );
    ...
    return Response.json(
      { error: "Choose a valid R2 action." },
      { status: 400, headers },
    );
  } catch (error) {
    const message =
      error instanceof Error && /UNIQUE/i.test(error.message)
        ? "That serial number is already in R2 processing."
        : error instanceof Error
          ? error.message
          : "The R2 record could not be saved.";
