...
type Deal={deal_number:string;public_lines?:PublicLine[];spreadsheet_filename?:string};
...
export async function GET(_request:Request,{params}:{params:Promise<{dealNumber:string}>}){
 ...
 try{
  const response=await supabaseRequest(`/rest/v1/pdd_public_deals?select=deal_number,public_lines,spreadsheet_filename&deal_number=eq.${encodeURIComponent(deal)}&published=eq.true&limit=1`);
  ...
  const {bytes,filename}=buildBidSpreadsheet(record.deal_number,lines.length,[...headers.map(header=>({header,value:(index:number)=>lines[index]?.values?.[header]||""})),{header:"Qty",value:(index:number)=>lines[index]?.quantity||0}],record.spreadsheet_filename,{awardMode:lines.some(line=>line.award_mode==="multiple")?"multiple":"single"});
