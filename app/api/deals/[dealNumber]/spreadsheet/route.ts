...
export async function GET(_request:Request,{params}:{params:Promise<{dealNumber:string}>}){
 ...
 try{
  ...
  const photo=await env.DB.prepare("SELECT object_key,content_type FROM deal_photos WHERE deal_number=? ORDER BY sort_order,created_at LIMIT 1").bind(record.deal_number).first<{object_key:string;content_type:string}>();
