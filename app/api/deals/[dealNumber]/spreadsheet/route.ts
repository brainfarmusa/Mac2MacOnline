...
export async function GET(_request:Request,{params}:{params:Promise<{dealNumber:string}>}){
 ...
 try{
  const response=await supabaseRequest(`/rest/v1/pdd_public_deals?select=deal_number,public_lines,spreadsheet_filename,closes_at,quantity,title,category&deal_number=eq.${encodeURIComponent(deal)}&published=eq.true&limit=1`);
