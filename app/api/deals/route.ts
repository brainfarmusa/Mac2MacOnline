...
export async function GET(){
  ...
  try{
    ...
    const response=await supabaseRequest(`/rest/v1/pdd_deals?select=${columns}&published=eq.true&status=eq.open&closes_at=gt.${encodeURIComponent(new Date().toISOString())}&order=closes_at.asc`);
