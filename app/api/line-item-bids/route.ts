...
export async function POST(request:Request){
  try{
    ...
    if(dealNumber!=="B082126-01"||!company||!contactName||!email||!phone||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return Response.json({error:"Please complete the company and contact information."},{status:400});
    if(!Array.isArray(body.lineItems)||body.lineItems.length<1||body.lineItems.length>181)return Response.json({error:"Enter a bid on at least one line item."},{status:400});
    ...
    if(lineItems.some(line=>!Number.isInteger(line.quantity)||line.quantity!==1||!Number.isFinite(line.unitBid)||line.unitBid<=0))return Response.json({error:"Each selected line must have a valid bid price."},{status:400});
    ...
    const internalBidNumber=`IB-B082126-01-${stamp}`;
    const db=env.DB;
