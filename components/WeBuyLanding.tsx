...
export default function WeBuyLanding({category,headline,intro,items,details,source,guideHref,guideTitle}:WeBuyLandingProps){
  const related=[
    ["Sell server RAM","/we-buy-ram"],
    ["Sell enterprise SSDs","/we-buy-ssds"],
    ["Sell GPUs & AI accelerators","/we-buy-gpus"],
    ["Sell Apple equipment","/we-buy-apple-equipment"],
    ["Sell servers & networking","/sell-used-servers-networking"],
    ["Sell CPUs & processors","/sell-used-cpus-processors"],
    ["Sell optical transceivers","/sell-optical-transceivers-network-components"],
    ["Sell business computer fleets","/sell-business-laptops-desktops-workstations"],
    ["View everything we buy","/equipment-we-buy"],
  ];
  return <Shell><main className="partsSellPage">
    ...
    <nav className="ramSsdCategoryLinks wrap" aria-label="Related equipment buying pages">{related.map(([label,href])=><Link href={href} key={href}>{label} →</Link>)}</nav>
